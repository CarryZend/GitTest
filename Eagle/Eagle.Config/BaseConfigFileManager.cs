        ...
        public new static string ConfigFilePath
        {
            get
            {
                if (filename == null)
                {
                    ...
                    if (context != null)
                    {
                        filename = context.Server.MapPath("~/ENT.config");
                    ...
                    {
                        filename = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "ENT.config");
                    }

                    if (!File.Exists(filename))
                    {
                        throw new ENTException("发生错误: 虚拟目录或网站根目录下没有正确的ENT.config文件");
                    }
                }
                return filename;
            }
